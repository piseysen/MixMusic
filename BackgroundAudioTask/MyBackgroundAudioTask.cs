    ...
    public sealed class MyBackgroundAudioTask : IBackgroundTask
    {
                ...
                if (!playbackStartedPreviously)
                {
                    ...
                    if (currentTrackId != null)
                    {
                        ...
                        var index = playbackList.Items.ToList().FindIndex(item =>
                            GetTrackId(item).ToString() ==currentTrackId.ToString());
        ...
        void BackgroundMediaPlayer_MessageReceivedFromForeground(object sender, MediaPlayerDataReceivedEventArgs e)
        {
            ...
            if(MessageService.TryParseMessage(e.Data, out trackChangedMessage))
            {
                try
                {
                    var index = playbackList.Items.ToList().FindIndex(i => (string)i.Source.CustomProperties[TrackIdKey] == trackChangedMessage.TrackId.ToString());
        ...
        void CreatePlaybackList(IEnumerable<MusicModel.Result> songs)
        {
            ...
            {
                var source = MediaSource.CreateFromUri(new Uri(song.music_path));
                source.CustomProperties[TrackIdKey] = song.music_path;
                source.CustomProperties[TitleKey] = song.music_title;
                source.CustomProperties[AlbumArtKey] =song.image_album;
